...
def telegram_ai_bulten_gonder(tarih_str, saat_str):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT fixture_id, saat, lig, ev_sahibi, deplasman, ust25, kg_var, ai_ust25_olasilik, ai_kgvar_olasilik, ai_tahmin
        FROM maclar 
        WHERE tarih = ? AND durum = 'NS' AND saat >= ? AND ai_tahmin IS NOT NULL AND bildirildi = 0
        ORDER BY saat ASC
    """, (tarih_str, saat_str))
    maclar = cursor.fetchall()
    if not maclar:
    ...
    baslik = f"🤖 <b>AI FOOTBALL BÜLTENİ</b> ({tarih_str})\n⏰ <i>Saat {saat_str} Sonrası Oynanacak {len(maclar)} Maç</i>\n=============================\n\n"
    parca_mesaj = baslik

    for m in maclar:
        fid, saat, lig, ev, dep, ust25, kg_var, ai_ust, ai_kg, tahmin = m
        mac_metni = (
            f"⏰ <b>{saat}</b> | 🏆 <code>{lig}</code>\n"
            f"⚔️ <b>{ev} vs {dep}</b>\n"
